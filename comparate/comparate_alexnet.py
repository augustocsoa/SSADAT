...
def classification_images():
    os.system(
        r"python3 comparate/alexnet_test.py --load_config comparate/alexnet_comparate.yaml"
    )
...
def reconstroi_videos():
    ...
    if not os.path.isdir("comparate/short_clips_nn/"):
        print("The rec_videos directory does not exist! creating..\n")
        os.makedirs("comparate/short_clips_nn/")

    in_dir = "comparate/short_clips/"
    out_dir = "comparate/short_clips_nn/"
    out_dir2 = "comparate/short_clips_final/"
    copy_tree(in_dir, out_dir)
    copy_tree(in_dir, out_dir2)

    all_short_clips = sorted(glob.glob(os.path.join("comparate/short_clips_nn/", "*")))
    ...
    for clip in all_short_clips:
        ...
        mark = np.load("comparate/saidas_rede/short_" + str(num_short_clips) + ".npy")
    ...
    os.system(r"python3 make_videos.py --load_config comparate/make_videos_nn.yaml")
